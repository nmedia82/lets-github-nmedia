...
class NM_Framwork_V2_nm_opw{
	...
	function load_scripts(){
		...
		if($this -> plugin_scripts){
			foreach($this -> plugin_scripts as $script){
				...
				if( $script['type'] == 'js'){
					wp_enqueue_script($this->plugin_meta['shortname'].'-'.$script['script_name'], $this->plugin_meta['url'].$script['script_source'], $script['depends'], $script['version'], $script['in_footer']);
	...
	*/
	function do_callbacks(){
		...
		foreach ($this -> ajax_callbacks as $callback => $viewer){
	...
	*/
	function get_rows_data($select, $where_data = NULL, $switch="AND", $debug = false)
	{
		...
		if($where_data != NULL){
			...
			$where = ' WHERE ';
			$format = array();
			foreach ($where_data as $type => $field_meta){
				...
				foreach ($pair as $f => $d){
						$where .= " ".$meta['field']." ".$meta['operator']." ".$wp_format;
						$format[] = $meta['value'];
				}
