...
<h2>
	<?php printf(__("%s", 'nm-opw'), $this->plugin_meta['name']);?>
</h2>
<div id="nm_opw-tabs" class="wrap">
		...
		<?php foreach($options['meat'] as $key => $data){
			...
			?>
				<tr valign="top" class="<?php echo $key; ?>">
					<th scope="row" width="25%"><?php printf(__('%s', 'nm-opw'), $label);?>
	...
	?>
	<p class="the-button"><button class="button button-primary"><?php _e('Save settings', 'nm-opw')?></button><span class="nm-saving-settings"></span></p>
	</form>
